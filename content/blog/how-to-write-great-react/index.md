...
如果组件代码都是干净整洁的，你的团队可以更快的行动。

这样就能保证一个好的应用了吗？不，架构的其他部分可能是一团糟。
...
在第一种情况下，把组件视为函数很简单。我们开始讲组件概念化为一个实体，并受不同的规则约束。

使用 class 组件时，我们更容易忘记组件的核心部分是 render 方法：一个返回 UI 片段的函数。

当我们忘记把组件视为函数时，我们会创建一个大且难理解的组件。它做了太多事，接受太多 props，或者有太多的分支条件。它很难使用或改进，真是令人头疼。
...
我们先把 React 放一边，思考一个问题：**什么才是好的函数？**
...
我们依次看一些这些规则，以及他们对 React 组件意味着什么。
...
组件主体不超过 50 行是个好的经验法则( class 组件的  render 方法)。如果查看文件的行数更方便，那大部分组件文件都不应该超过 250 行。100 行以内是最理想的。

**保证你的组件小巧。**

## 组件应该只做一件事

关于这个主题，我写了一篇更详细的文章：Tiny Components: What could go wrong?

简而言之，你的组件应该仅承担一项主要职责。

如果你因为要调整菜单条目的顺序而修改 `MenuList.jsx` 组件，这很好。如果你因为要调整侧边栏的打开方式而修改 `MenuList.jsx`，那很糟糕。

将你的 UI 拆成多个小块，每个小块仅处理一件事。
...
`Dashboard` 组件做的事太多了。有太多的理由去修改它了，并且由于缺乏抽象，它很难读。
...
顺便说一下，布尔型的 props 会增加不必要的复杂性。Filip Danić 有一篇很好地文章来讲述这个主题。
...
事实上，很难给组件命名时，说明这个组件做了太多的事。答案是：这个组件的作用是什么？这应该很简单，然后顺其自然地给组件一个描述性的名字。

如果一个开发者看了你的应用的组件数，他应该对每个组件做什么有完整和清晰的了解。没什么意外。
